...
Legacy **`/exchange`** → `/markets`, **`/vault/submit/mint`** → `/vault/submit`, **`/login`** and **`/signup`** → `/` via `next.config.ts` (query string is kept). Sign-in is Privy’s client modal (`openSignIn` / header Sign up), not a dedicated page.
...
| `/vault` | `app/vault/page.tsx` | **Sell hub** — signed-out landing / empty dashboard (zero tabs, send-a-card + browse markets) / **vaulting list** (tabs: All · Shipped · Verifying · Vaulted · Rejected; `Vault-Dashboard-Active.html`) |
...
| `/marketplace/collections/[collectionKey]` | `app/marketplace/collections/[collectionKey]/page.tsx` | Collection — trade panel, order book, dual chart (`?listing=` focuses a copy) |
...
| `/marketplace/admin/markets` | `app/marketplace/admin/markets/page.tsx` | Admin — home landing preview |
...
| Sign-in (header / `openSignIn`) | Privy SDK modal → `POST /api/auth/privy/session` |
| `/settings` | Session user + prefs (`PATCH /api/auth/profile`); address book (`/api/user/shipping-addresses`); USDC + Add funds (MoonPay); KYC via `/kyc`; delete via `POST /api/auth/delete-account`. Guests get Privy modal then `/`. |
| `/profile` | Redirect → `/settings` |
| `/site-access` | `POST /api/site-access/verify` |
| `/marketplace/collections/[collectionKey]` | Collection detail, cardhedger, market-series, stats, ai-insight, Seaport orders (`?listing=` focuses a copy) |
