...
4. **Prices** — `POST …/portfolio-market-batch` reads `collection_market_snapshots` when a collection bucket exists. Tokens **without** a collection row (or with an unmatched snapshot) fall back to `POST …/cardhedger/mint-previews` for spot price and `GET …/rwa/:tokenId/trades` for the gallery sparkline. Missing snapshots still enqueue a background refresh; the query polls every 20s until `syncedAt` appears.
