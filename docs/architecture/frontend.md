...
4. **Prices** — snapshot marks from assets-page BFF. The BFF preloads the full `collection_market_snapshots` price index (TTL cache) in parallel with metadata, then joins owned tokens by `collection_key` (registry or metadata bucket). Cardhedger mint-previews load in a **follow-up** request only for tokens still without snapshot prices. Gallery sparklines come from snapshot series. Hero **Portfolio value** sums live marks on loaded visible rows; **24h P/L chip** uses daily snapshots. Missing snapshots enqueue background refresh.
