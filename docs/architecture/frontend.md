...
Tokenable JWT sync still runs via `PrivySessionBridge`; profile page and marketplace routes use `useAuthStore` as before. Wallet-only (MetaMask) accounts that still have a `@privy.wallet` placeholder are prompted for a real contact email via `AddEmailRequiredModal` (saved through `PATCH /auth/profile`). During the KBW window (`isKbwEventActive` / `NEXT_PUBLIC_KBW_EVENT_START|END`), `/event` Stage 2 opens Privy login (`openSignIn({ returnTo: "/" })`), then lands on main (`/`) and arms `KbwMysteryOfferModal`. Offer visibility SSOT is the portfolio mystery card: **card still present** (`GET …/kbw-mystery-card/:wallet` → `burned: false`) → show on `/`; **card removed** (`burned: true`) → never show. **BUY • FREE** navigates to `/portfolio?tab=assets`. Stage 1 Instagram only unlocks Stage 2. The synthetic KBW Mystery Card in Portfolio is **email-scoped**: one contact email may hold it across wallets until burn; burning hides it for every wallet that shares that email (`kbw_mystery_card_burns`).
