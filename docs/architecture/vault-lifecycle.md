...
1. Portfolio My Assets → open a card → Certificate of Ownership (`/portfolio/assets/[tokenId]`) → **Redeem** (owner, no open redemption on **that** token). An active ask on the same token is cancelled at pay (`cancelActiveOrdersForTokens`) — the Certificate button stays enabled. One card per draft; the batch model still supports up to 50 if another entry point is added. `GET /rwa/redemptions/mine` joins `rwa_tokens` by vault cycle **and** PSA cert (not every token that shares a stale `vault_cycle_id`), so sibling copies of the same card stay independently redeemable. When a redemption is already open **for this token** the same button becomes **Redemption status** and deep-links to the matching `/portfolio/redeem?view=&batch=` screen (`?state=` from the HTML prototype is rewritten to `?view=`). There is no `view=pay` bookmark — a charged request opens preparing / transit / resume, not Review and pay.
...
3. **Review and pay** → Review and pay screen
