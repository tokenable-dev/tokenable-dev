...
For card offers (bids): `side: "bid"`, `collectionKey`, offer itemType `1` (USDC). **Token offer:** real `tokenId`, consideration itemType `2` (ERC721 for that token). **Collection Place Bid:** `tokenId` `0`, consideration itemType `4` (ERC721_WITH_CRITERIA; Merkle root of minted ids in the bucket). **An active ask is not required.** If the bid amount is **greater than or equal to** a live ask in the same collection (including the bidder’s own listing), the API rejects with `409` and `BID_CROSSES_ASK` — the client must fulfill that ask at the listed price instead of posting a bid. Collection-detail Bid tab shows **Buy now** (not Place bid) when the typed amount meets or exceeds that live floor. Active bids per wallet per collection are **unlimited** (`MARKETPLACE_MAX_ACTIVE_BIDS_PER_OFFERER=0`; set to `1` to restore the old cap). Bids expire after a buyer-chosen **1 / 3 / 7 / 14 / 30 / 60 / 90 / 180 day** window (Seaport `endTime − startTime`). Default in the Place Bid UI is **7 days**. Other durations are rejected.
