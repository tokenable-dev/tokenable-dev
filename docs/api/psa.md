...
- **Create:** best candidate is stored on first collection insert.
...
**Manual override:** `POST /api/marketplace/collections/:key/admin/cover` or `admin/cover/from-token` (`save` = force; `upgradeIfBetter` = score-based upgrade).
