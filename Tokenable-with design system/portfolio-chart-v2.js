...
function drawChart(){
  ...
  var elChg = document.getElementById('pf-chart-change');
  if(elTotal) elTotal.textContent = fmtPrice(total);
  if(elChg){
    var sign = chg>=0?'+':'';
    elChg.textContent = sign+fmtPrice(Math.abs(chg))+' ('+sign+chgPct+'%)';
    elChg.style.color = chg>=0?'#00C350':'#E4374A';
  }
...
var pollId=setInterval(function(){
  ...
  if(svg||tries++>30){ clearInterval(pollId); init(); }
},300);
