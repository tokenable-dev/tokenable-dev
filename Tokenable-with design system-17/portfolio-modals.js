...
/* Sellers price against this number, so it reads as a figure, not a caption. */
function mktBlock(v){
  return '<div style="display:flex;align-items:baseline;justify-content:space-between;gap:12px;'
    + 'margin:0 0 18px;padding:12px 14px;background:rgba(255,255,255,0.05);border-radius:10px;">'
    + '<span style="font-family:var(--font-mono);font-size:10px;letter-spacing:0.1em;text-transform:uppercase;'
    +   'color:rgba(255,255,255,0.5);font-weight:600;">Current market value</span>'
    + '<span style="font-family:var(--font-mono);font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.01em;">$'
    + fmtPrice(v) + '</span></div>';
}
...
window.pfSetPriceModal = function(name, mktVal, isListed, listPrice, hasBids, highestBid, lowAsk){
  listPrice = listPrice || mktVal;
  ...
  // Demo market: the lowest live ask sits just under market value. A caller
  // that knows there are no asks passes 0 and that reference drops out.
  var lowestAsk = (lowAsk === undefined || lowAsk === null)
    ? (mktVal ? Math.round(mktVal * 0.97) : 0) : lowAsk;
  var lastSold = highestBid > 0 ? highestBid : 0;

  var listedHeader = isListed ?
    '<div style="display:flex;align-items:baseline;justify-content:space-between;gap:12px;padding:12px 14px;background:rgba(26,111,255,0.10);border-radius:10px;margin-bottom:16px;">' +
      '<span style="'+labelStyle+'margin:0;color:rgba(255,255,255,0.6);">Currently listed at</span>' +
      '<span style="font-family:var(--font-mono);font-weight:700;font-size:18px;color:#fff;">$'+fmtPrice(listPrice)+'</span>' +
    '</div>' : '';

  openModal(
    '<div style="'+labelStyle+'">' + (isListed ? 'Edit price' : 'Set price') + '</div>' +
    '<div style="font-size:16px;font-weight:600;color:#fff;margin:8px 0 14px;">' + name + '</div>' +
    listedHeader +
    TKPrice.html({
      ns:'pfsn', tone:'dark',
      market:mktVal, lowestAsk:lowestAsk, lastSold:lastSold,
      value: isListed ? listPrice : 0
    }) +
    '<p style="font-size:12px;color:rgba(255,255,255,0.4);margin:14px 0 0;line-height:1.5;">' +
      (isListed ? 'The new price applies as soon as you update it.'
                : 'Your card goes live at the price you set.') + '</p>',
    '<button id="pfsn-cta" style="'+primaryBtnStyle+'width:100%;">' + (isListed ? 'Update price' : 'List for sale') + ' \u2192</button>' +
    '<button style="'+ghostBtnStyle+'margin-top:8px;width:100%;" onclick="closeModal()">Cancel</button>'
  );
  ...
  function syncCta(v){
    var ok = v > 0;
    if(cta){ cta.disabled = !ok; cta.style.opacity = ok ? '1' : '0.45'; cta.style.pointerEvents = ok ? 'auto' : 'none'; }
  }
  TKPrice.mount('pfsn', { onChange: syncCta });
  syncCta(TKPrice.value('pfsn'));

  if(cta) cta.onclick = function(){
    var v = TKPrice.value('pfsn');
    if(v > 0) pfSaleResult(isListed ? 'updated' : 'listed', v);
  };
};
...
window.pfSaleResult = function(kind, price){
  ...
  else if(kind==='listed') cfg={color:'#00C864',bg:'rgba(0,200,100,0.12)',rad:'50%',icon:'<polyline points="20 6 9 17 4 12"/>',title:'Listed at $'+fmtPrice(price),sub:'Your card is live. You\u2019ll receive $'+fmtPrice(net)+' when it sells.'};
  ...
  sov.innerHTML='<div style="text-align:center;width:340px;max-width:calc(100vw - 48px);background:#1e1e2e;border-radius:16px;padding:32px 28px 28px;box-shadow:0 24px 64px rgba(0,0,0,0.6);animation:pfSuccessPop 0.5s ease forwards;">' +
...
window.pfConfirmAction = function(action){
  /* Listing writes to the network, so the fee is charged here — not at vaulting. */
  if((action==='listed' || action==='listing updated') && window.tkGas && !window.__pfGasOk){
    window.tkGas.require(1, function(){ window.__pfGasOk=true; window.pfConfirmAction(action); window.__pfGasOk=false; });
    return;
  }
  ...
  sov.innerHTML = '<div style="text-align:center;width:300px;max-width:calc(100vw - 48px);background:#1e1e2e;border-radius:16px;padding:32px 28px 28px;box-shadow:0 24px 64px rgba(0,0,0,0.6);animation:pfSuccessPop 0.5s ease forwards;">' +
...
document.addEventListener('click', function(e){
  ...
  switch(action){
    ...
    case 'withdraw':
    case 'redeem': window.location.href = 'Ship-From-Vault.html'; break;
