...
var TAX = [
  { id:'sports', label:'Sports', count:386540, kids:[
...
/* Flat "popular category" chips (spec §1) — concrete categories only, no abstract
   groups. `All` prepended in the renderer. Order: Sports first, then Gaming. */
var CAT_CHIPS = [
  { id:'sports/basketball', label:'Basketball' },
  { id:'sports/baseball',   label:'Baseball' },
  { id:'sports/football',   label:'Football' },
  { id:'sports/soccer',     label:'Soccer' },
  { id:'tcg/pokemon',       label:'Pok\u00e9mon' },
  { id:'tcg/onepiece',      label:'One Piece' },
  { id:'tcg/magic',         label:'Magic' },
...
function facetsFor(cat){
  if(!cat) return ['grade','year','price','variant','vault','seller'];
  if(cat.indexOf('tcg/pokemon')===0) return ['set','character','rarity','grade','year','price','variant','vault','seller'];
  if(cat.indexOf('tcg/magic')===0) return ['set','rarity','grade','year','price','variant','vault','seller'];
  if(cat.indexOf('sports')===0) return ['player','team','set','grade','year','price','variant','vault','seller'];
  if(cat.indexOf('tcg')===0) return ['set','grade','year','price','variant','vault','seller'];
  return ['grade','year','price','variant','vault','seller'];
}
...
var FACET_META = {
  ...
  grade:     { label:'Grade',     kind:'chips', values:['PSA 10','PSA 9','BGS Pristine','BGS 10','BGS 9.5'] },
  vault:     { label:'Vault',     kind:'chips', values:['PSA VAULT','CYD VAULT'] },
  variant:   { label:'Variant',   kind:'chips', values:['Holo','Reverse Holo','1st Edition','Refractor','Autograph'] },
  seller:    { label:'Seller',    kind:'chips', values:['Tokenable','Verified partner','Individual'] },
...
var LIST_KEYS = ['set','character','player','team','rarity','grade','vault','variant','seller'];
var expanded = {};
...
/* Flat popular-category chips (spec §1): All + concrete categories, then an
   "All categories" trigger that opens the grouped Category popover. A chip is
   active when it IS the selected category or an ancestor of it (e.g. Pokémon
   chip stays on while drilled into Pokémon › English). */
function catChipsHTML(){
  // Only the selected category chip is shown (plus All when nothing selected).
  if(!S.cat) return '<button class="mk-catchip on" data-cat="">All</button>';
  var match = null;
  CAT_CHIPS.forEach(function(c){ if(S.cat === c.id || S.cat.indexOf(c.id+'/')===0) match = c; });
  var out = ['<button class="mk-catchip" data-cat="">All</button>'];
  if(match) out.push('<button class="mk-catchip on" data-cat="'+match.id+'">'+match.label+'</button>');
  return out.join('');
}
...
function chipsHTML(key, values, counts){
  if(key === 'grade') return gradeChipsHTML(values, counts);
...
/* Grades group by grading company — PSA and BGS scales don't compare 1:1,
   so a flat chip row reads as one scale when it's really two. */
function gradeChipsHTML(values, counts){
  var groups = {}, order = [];
  values.forEach(function(v){
    var co = v.split(' ')[0];
    if(!groups[co]){ groups[co] = []; order.push(co); }
    groups[co].push(v);
  });
  return order.map(function(co){
    return '<div class="mk-fgroup">'
      + '<div class="mk-fgroup-lbl">'+co+'</div>'
      + '<div class="mk-fchips">'+groups[co].map(function(v){
          var sel = S.grade.indexOf(v)>=0;
          var c = counts && counts[v];
          return '<button class="mk-fchip'+(sel?' on':'')+'" data-fchip="grade" data-val="'+v+'">'+v+(c?' <i>'+fmt(c)+'</i>':'')+'</button>';
        }).join('')+'</div>'
    + '</div>';
  }).join('');
}
...
function cardData(c){
  ...
  c.__d = {
    ...
    seller: c.getAttribute('data-seller')||'',
...
var SOFT_KEYS = { variant:1, seller:1 };
function ok(d){
  if(S.cat && d.cat.indexOf(S.cat)!==0) return false;
  for(var i=0;i<LIST_KEYS.length;i++){
    var k = LIST_KEYS[i];
    if(!S[k].length) continue;
    if(SOFT_KEYS[k] && !d[k]) continue;
    if(S[k].indexOf(d[k])<0) return false;
  }
...
function apply(persist){
  ...
  var cc = $('mkCatChips'); if(cc) cc.innerHTML = catChipsHTML();
  var ptree = $('mkPopTree'); if(ptree) ptree.innerHTML = treeHTML(TAX, 0);
  var pg = $('mkPopGradeBody'); if(pg) pg.innerHTML = chipsHTML('grade', FACET_META.grade.values);
  var pp = $('mkPopPriceBody'); if(pp) pp.innerHTML = rangeHTML('price');
  syncBar();
...
/* ---------- Slim bar: labels mirror the shared filter state ---------- */
...
function priceLabel(){
  ...
  if(S.priceMax) return 'Under '+money(S.priceMax);
  return '';
}
function activeCount(){
  var n = 0;
  LIST_KEYS.forEach(function(k){ n += S[k].length; });
  if(S.cat) n++;
  if(S.yearMin || S.yearMax) n++;
  if(S.priceMin || S.priceMax) n++;
  return n;
}
function setChip(popId, labelId, text, on){
  var lbl = $(labelId); if(lbl) lbl.textContent = text;
  var trig = document.querySelector('[data-pop="'+popId+'"]');
  if(trig) trig.classList.toggle('on', !!on);
}
function syncBar(){
  setChip('cat', 'mkLblCat', S.cat ? LABEL[S.cat].label : 'Category', S.cat);
  setChip('grade', 'mkLblGrade',
    S.grade.length ? S.grade[0] + (S.grade.length>1 ? ' +'+(S.grade.length-1) : '') : 'Grade',
    S.grade.length);
  var pl = priceLabel();
  setChip('price', 'mkLblPrice', pl || 'Price', pl);
  var mc = $('mkMoreCount');
  if(mc){
    var n = activeCount();
    mc.textContent = n;
    mc.style.display = n ? 'inline-block' : 'none';
  }
}
function closePops(except){
  document.querySelectorAll('.mk-pop').forEach(function(p){
    if(p !== except) p.classList.remove('open');
  });
  document.querySelectorAll('[data-pop]').forEach(function(b){
    var id = 'mkPop' + b.getAttribute('data-pop').charAt(0).toUpperCase() + b.getAttribute('data-pop').slice(1);
    b.classList.toggle('open', $(id) === except);
  });
}
...
document.addEventListener('click', function(e){
  ...
  var popBtn = t.closest && t.closest('[data-pop]');
  if(popBtn){
    var key = popBtn.getAttribute('data-pop');
    var panel = $('mkPop' + key.charAt(0).toUpperCase() + key.slice(1));
    closePops(panel && panel.classList.contains('open') ? null : panel);
    if(panel && !panel.classList.contains('open')) panel.classList.add('open');
    closeTA(); e.preventDefault(); return;
  }
  if(t.closest && t.closest('[data-popclose]')){ closePops(null); e.preventDefault(); return; }
  ...
  var row = t.closest && t.closest('.mk-tree-row[data-cat], .mk-crumb [data-cat], .mk-catchip[data-cat]');
  if(row){
    ...
    pickerQ = {};
    if(S.cat) S.cat.split('/').reduce(function(a,seg){ a.push(seg); expanded[a.join('/')] = true; return a; }, []);
    closeTA(); closePops(null); apply(); e.preventDefault(); return;
  }
  ...
  if(!t.closest || !t.closest('.mk-fw')) closePops(null);
}, false);

document.addEventListener('input', function(e){
  var t = e.target;
  if(t.id === 'mkTreeSearch' || t.id === 'mkPopTreeSearch'){
    treeQuery = t.value;
    var tr=$('mkTree'); if(tr) tr.innerHTML = treeHTML(TAX,0);
    var pt=$('mkPopTree'); if(pt) pt.innerHTML = treeHTML(TAX,0);
    var other = $(t.id === 'mkTreeSearch' ? 'mkPopTreeSearch' : 'mkTreeSearch');
...
document.addEventListener('keydown', function(e){
  if(e.key === 'Escape'){ closeTA(); closePops(null); document.body.classList.remove('mk-side-on'); }
...
/* The host re-renders the results grid continuously, so a card's <a> can be
   replaced between pointerdown and pointerup — the browser then fires no click
   at all and the link looks dead. Navigate from the pointer pair instead. */
(function(){
  var downHref = null, downAt = 0;
  function cardHref(el){
    var a = el && el.closest && el.closest('a.card');
    if(!a || (el.closest && el.closest('.fav'))) return null;
    return a.getAttribute('href');
  }
  document.addEventListener('pointerdown', function(e){
    if(e.button) return;
    downHref = cardHref(e.target);
    downAt = Date.now();
  }, true);
  document.addEventListener('pointerup', function(e){
    var href = downHref; downHref = null;
    if(!href || Date.now() - downAt > 900) return;
    var el = document.elementFromPoint(e.clientX, e.clientY);
    if(cardHref(el) !== href) return;
...
/* ---------- Mobile: search collapses to an icon that opens a full-screen modal ---------- */
(function(){
  function openSearchModal(){
    var modal = document.getElementById('mkSearchModal');
    var box = document.getElementById('mkUsearchBox');
    var head = modal && modal.querySelector('.mk-search-modal-head');
    if(!modal || !box || !head) return;
    modal.insertBefore(box, head.nextSibling);
  ...
  function closeSearchModal(){
    var modal = document.getElementById('mkSearchModal');
    var box = document.getElementById('mkUsearchBox');
    var anchor = document.getElementById('mkUsearchAnchor');
    if(!modal || !box || !anchor) return;
    anchor.parentNode.insertBefore(box, anchor.nextSibling);
    modal.classList.remove('open');
  }
  document.addEventListener('click', function(e){
    if(e.target.closest && e.target.closest('#mkSearchIconBtn')){ openSearchModal(); return; }
    if(e.target.closest && (e.target.closest('#mkSearchModalClose') || e.target === document.getElementById('mkSearchModal'))){ closeSearchModal(); return; }
  });
  window.addEventListener('resize', function(){
    if(window.innerWidth > 820) closeSearchModal();
  });
  document.addEventListener('keydown', function(e){
...
function panelStale(){
  var tr = $('mkTree');
  if(!tr || !tr.querySelector('.mk-tree-row')) return true;
  // The bar paints after the drawer, so its popovers can still be empty.
  var pt = $('mkPopTree'), pg = $('mkPopGradeBody'), pp = $('mkPopPriceBody');
  if(pt && !pt.firstChild) return true;
  if(pg && !pg.firstChild) return true;
  if(pp && !pp.firstChild) return true;
  return false;
}
function sync(){
