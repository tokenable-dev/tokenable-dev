...
(function(){
  ...
  window.tkPortfolioHref = function(hash){
    return (partner ? 'Partner-Portfolio.html' : window.tkPortfolioHref()) + (hash||'');
  };
})();
...
(function(){
  ...
  css.textContent = [
    '.tkm-overlay{display:none;position:fixed;inset:64px 0 0 0;z-index:8000;background:rgba(0,0,0,0.4);}',
    '.tkm-overlay.open{display:block;}',
    '.tkm-drawer{position:fixed;top:64px;right:0;bottom:0;width:100vw;z-index:8001;background:#111;display:flex;flex-direction:column;transform:translateX(100%);transition:transform 0.3s cubic-bezier(0.4,0,0.2,1);overflow-y:auto;-webkit-overflow-scrolling:touch;}',
    '.tkm-drawer.open{transform:translateX(0);}',
    '.tkm-drawer{padding-top:12px;}',
  ...
  drawer.innerHTML = [
    ...
    '    <div class="tkm-profile__info" style="flex:1;"><span style="display:flex;align-items:center;gap:6px;"><span class="tkm-profile__addr">0xF4\u20269aE2</span><button class="tk-copy-addr" type="button" aria-label="Copy address" title="Copy address" data-addr="0xF4b93C1a7e2D5f08A3c6E19b4D7a02c58Ce29aE2" style="width:24px;height:24px;flex:none;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.08);border:0;border-radius:6px;color:rgba(255,255,255,0.6);cursor:pointer;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></span><span class="tkm-profile__kyc"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Verified</span></div>',
    ...
    '  <a class="tkm-item" href="Settings.html">' + icons.settings + 'Settings</a>',
    '</nav>',
    '<div class="tkm-divider"></div>',
  ...
  function burger(on){
    document.querySelectorAll('.gnb-burger').forEach(function(b){
  ...
  document.addEventListener('click', function(e){
    ...
    if(e.target.closest('.gnb-burger')){
      e.preventDefault(); e.stopPropagation();
      var old=document.querySelector('.gnb-drawer'); if(old) old.classList.remove('open');
      if(drawer.classList.contains('open')) closeDrawer(); else openDrawer();
    }
    if(e.target.closest('#tkm-logo')) closeDrawer();
    if(e.target.closest('#tkm-notifications')){
      e.preventDefault();
      /* hand the same surface over to the notification panel: drop the drawer without
         its slide-out so the panel appears to advance in place, not stack on top */
      var dr=document.querySelector('.tkm-drawer'), ov=document.querySelector('.tkm-overlay');
      if(dr){ dr.style.transition='none'; dr.classList.remove('open'); requestAnimationFrame(function(){ dr.style.transition=''; }); }
      if(ov) ov.classList.remove('open');
      burger(false);
      document.documentElement.style.overflow=''; document.body.style.overflow='';
      if(window.tkOpenNotifications) tkOpenNotifications();
    }
...
/* ---- Copy wallet address (delegated; menus are re-injected) ---- */
(function(){
  if(window.__tkCopyAddrBound) return;
  window.__tkCopyAddrBound = true;
  document.addEventListener('click', function(e){
    var btn = e.target.closest && e.target.closest('.tk-copy-addr');
    if(!btn) return;
    e.preventDefault(); e.stopPropagation();
    ...
    function done(){
      ...
      btn.dataset.busy = '1';
      var prev = btn.innerHTML, pc = btn.style.color, pb = btn.style.background;
      btn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6"><polyline points="20 6 9 17 4 12"></polyline></svg>';
      btn.style.color = 'var(--pos,#00C350)'; btn.style.background = 'rgba(0,200,100,0.16)';
      setTimeout(function(){ btn.innerHTML = prev; btn.style.color = pc; btn.style.background = pb; btn.dataset.busy = ''; }, 1400);
    }
    function fallback(){
