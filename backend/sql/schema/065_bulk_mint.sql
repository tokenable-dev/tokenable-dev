-- Partner bulk mint + list jobs (admin Excel/CSV → prepare → mintBatch to partner → Seaport ask).
...
COMMENT ON TABLE bulk_mint_jobs IS
  'Admin partner bulk mint+list: Excel cert+price → PSA prepare → mintBatch to partner wallet → Seaport asks.';
COMMENT ON TABLE bulk_mint_job_items IS
  'Per-cert row for a bulk_mint_jobs run (includes list price and Seaport order_hash when listed).';
