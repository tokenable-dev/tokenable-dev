...
export function pickRwaAssetDisplayImageRef(
  ...
  const psa = graded?.psa as Record<string, unknown> | undefined;
  const cert =
    typeof psa?.certImageSourceUrl === 'string' ? psa.certImageSourceUrl.trim() : '';
 ...
 */
export function pickTrendingSlabImageRef(meta: Record<string, unknown>): string | null {
  const props = meta.properties as Record<string, unknown> | undefined;
  const graded = (props?.graded ?? meta.graded) as Record<string, unknown> | undefined;
  const psa = graded?.psa as Record<string, unknown> | undefined;
  const cert = typeof psa?.certImageSourceUrl === 'string' ? psa.certImageSourceUrl.trim() : '';
...
export function psaCertNumberFromGradedMeta(meta: Record<string, unknown>): string | undefined {
  const props = meta.properties as Record<string, unknown> | undefined;
  const graded = (props?.graded ?? meta.graded) as Record<string, unknown> | undefined;
  const psa = graded?.psa as Record<string, unknown> | undefined;
