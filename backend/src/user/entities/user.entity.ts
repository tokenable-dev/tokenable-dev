...
export class User {
  ...
  @Column({ name: 'platform_email_verified_at', type: 'timestamptz', nullable: true })
  ...
  @Column({ name: 'verification_email_last_sent_at', type: 'timestamptz', nullable: true })
  verificationEmailLastSentAt: Date | null;

  /** 체크섬 정규화 주소 (0x…). NULL 허용·UNIQUE — PostgreSQL에서 NULL은 중복 허용 */
  @Column({ name: 'wallet_address', type: 'varchar', length: 42, unique: true, nullable: true })
